...
void VulkanRenderer::setup_frame_graph() {
    ...
    vertex_buffer.add_vertex_attribute(VK_FORMAT_R32G32B32_SFLOAT, offsetof(OctreeVertex, position));
    vertex_buffer.add_vertex_attribute(VK_FORMAT_R32G32B32_SFLOAT, offsetof(OctreeVertex, color));
...
void VulkanRenderer::recreate_swapchain() {
    ...
    rendering_finished_semaphore =
        std::make_unique<wrapper::Semaphore>(vkdevice->get_device(), "Rendering finished semaphore");
    game_camera.set_perspective(45.0f, (float)window->get_width() / (float)window->get_height(), 0.1f, 256.0f);
    vkDeviceWaitIdle(vkdevice->get_device());
}
